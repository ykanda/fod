...
func TestDotfileFilterFilterAbsolutePath(t *testing.T) {
	entries := []*Entry{
		{Path: "../", Type: FsTypeDir},
		{Path: "/tmp/.hidden", Type: FsTypeFile},
		{Path: "/tmp/visible.txt", Type: FsTypeFile},
	}
	filter := &DotfileFilter{enable: true}
	got := filter.filter(entries)
	if len(got) != 2 {
	...
	for _, entry := range got {
		if entry.Path == "/tmp/.hidden" {
			t.Fatalf("DotfileFilter.filter() unexpectedly included hidden absolute path")
		}
	}
}
